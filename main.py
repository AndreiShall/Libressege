...
class Protocol():
    ...
    def getPublicKey(self):
        return self.crypto.getKey()

    def getAddress(self):
        return "ff"#open("/var/lib/tor/hidden_service/hostname", "r").read()[:5]
...
"border-top-right-radius: 0px;")
        ...
        self.protocol.start()
        print(self.protocol.getAddress())
        print(self.protocol.getPublicKey())
        ...
        x = threading.Thread(target=self.listsUpdater, args=())
        x.start()
    ...
    def newMessage(self):
        if self.recipientAddressLabel.text() != "":
            ...
            f.close()
            self.protocol.sendMessage(self.recipientAddressLabel.text(), self.messageInput.text(), open(join(join(os.getcwd(), "dialogs"), self.recipientAddressLabel.text()), "r").readlines()[0])
